...
界面按「调色 → 水印 → 导出」三步流程组织：上半屏固定为预览区 + 缩略图，下半屏为三栏工作区。

1. 点击 **缩略图条末尾的「＋」** 上传照片（最多 9 张）
2. 在缩略图列表选择要预览的图片
3. 底部 **「调色」栏**：选择分类与配方，调整浓度（默认 50%）；默认 sRGB 直查，需按 Rec.709 视频输入时切到「Rec.709 还原」
4. 底部 **「水印」栏**：选择水印、混合模式，调整不透明度/大小/位置
5. 底部 **「导出」栏**：选择格式 JPG/PNG 与 JPG 质量，点击「下载当前」或「下载全部 (ZIP)」
6. 预览区可点 **「🖼 原图 / ✨ 效果图」** 对比，始终显示当前图的最终合成效果（调色 + 水印）

### 关于 LUT 渲染模式

默认 **sRGB 直查**：直接对 sRGB 像素查表，与 Photoshop「颜色查找」一致，适合多数风格化配方。

若你使用的配方确为「真 Rec.709（视频显示信号）」输入（影视风格 LUT，等价达芬奇里 `dlog → Rec.709 → 套 LUT` 之后再套的那类），请在 LUT 面板切到 **Rec.709 还原**，其渲染链路为：
...
│   │   ├── ExportPanel.vue        # 导出面板（导出栏：格式/质量/下载）
│   │   ├── PreviewPanel.vue       # 预览画布（LUT + 水印合成）
│   │   └── ThumbPanel.vue         # 缩略图列表（末尾「+」添加，最多 9 张）
│   ├── composables/
